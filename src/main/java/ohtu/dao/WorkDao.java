...
public interface WorkDao extends Dao<Work, Integer> {
    ...
    /**
     * creates the data in the the object work into the database, then updates
     * its id to match that returned by the database
     *
    ...
    List<Work> searchByAuthor(String author);

    List<Work> searchByTitle(String title);

    List<Work> searchByUrl(String url);
